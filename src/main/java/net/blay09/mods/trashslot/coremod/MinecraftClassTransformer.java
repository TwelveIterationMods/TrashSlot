            ...
            if(method.name.equals(methodName) && method.desc.equals(METHOD_DESC)) {
                ...
                for(int i = 0; i < method.instructions.size(); i++) {
                    ...
                    if(node instanceof VarInsnNode) {
                        if(node.getOpcode() == Opcodes.ISTORE && ((VarInsnNode) node).var == 2) { // ISTORE 2
                            insertAfter = node;
                            break;
                        }
                    }
                ...
                if(insertAfter != null) {
                    ...
                    mn.visitIincInsn(2, -1); // IINC 2 -1
                    method.instructions.insert(insertAfter, mn.instructions);
                    logger.info("TrashSlot successfully patched {} in {}!", methodName, className);
                } else {
                    logger.warn("TrashSlot failed to patch {0}::{1} ({2} not found) - pick block in creative will be weird!", className, methodName, "ISTORE 2");
                }
            }
        }
